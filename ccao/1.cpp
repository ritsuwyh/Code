...
void RotateL(Node* parent)
{
  ...
  //注意孩子为空的情况
  parent->sub_tree_size=parent->_cnt;
...
void TRANSPLANT(Node *u,Node *v){
    if(u->_parent==NULL){
        if(v==NULL){
            ...
            _root->_cnt=1;
            _root->sub_tree_size=1;
	        _root->_parent=NULL;
            _root->_left=NULL;
            _root->_right=NULL;
            _root->_bf=0;//平衡因子 bf 变量(平衡因子 = 右子树高度 - 左子树高度)
        }else{
        _root=v;//u=v
        }
...
int AVLREMOVE(Node *_root,int z){
    ...
    if(cur->_cnt==0){
        if(cur->_left==NULL){
            TRANSPLANT(cur,cur->_right);
        }else if(cur->_right==NULL){
            TRANSPLANT(cur,cur->_left);
        }else{
            Node* y=AVLMIN(cur->_right);

            Node* temp=y;
            while(temp->_parent!=cur){
                temp=temp->_parent;
                temp->sub_tree_size-=y->_cnt;
            }
            ...
            y->_left=cur->_left;
            y->_left->_parent=y;
            y->sub_tree_size=cur->sub_tree_size;
...
int func(Node* _root,int upbound){
  ...
  while(p!=NULL){
    if(p->key<upbound){
      if(p->_left!=NULL){
      ans+=p->_left->sub_tree_size+p->_cnt;
      }else{
        ans+=p->_cnt;
      }
      p=p->_right;
...
int main(int argc, char** argv) {
  ...
  // }
    ...
    // cout<<AVLMIN(_root)->key<<" "<<AVLMIN(_root)->_cnt<<" "<<AVLMIN(_root)->sub_tree_size<<endl;
    cout<<"head"<<_root->key<<" "<<_root->_cnt<<" "<<_root->sub_tree_size<<endl;
