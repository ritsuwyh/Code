...
class Population:
    ...
    def cross(self, chrom1, chrom2):
        ...
        if chrom1 != chrom2 and p < self.crossover_probability:
            ...
            mask_high = n << t    # << 左移运算符
            (r1, r2) = (chrom1 & mask_high, chrom2 & mask_high)   # & 按位与运算符：参与运算的两个值,如果两个相应位都为1,则该位的结果为1,否则为0
            mask_low = n >> (self.chromosome_size - t)
            (l1, l2) = (chrom1 & mask_low, chrom2 & mask_low)
    ...
    def mutate(self, chrom):
        ...
        if p < self.mutation_probability:
            ...
            mask1 = 1 << (t - 1) 
            num = chrom & mask1 
            if num > 0: # 要变异的那个位上面是1 应变为0
                chrom = chrom & (~num)  # ~ 按位取反运算符：对数据的每个二进制位取反,即把1变为0,把0变为1 
            else: # 是0 应变为1
    ...
    def reproduct_elitist (self):
        ...
        if (j >= 0):
            self.elitist['chromosome'][0] = self.decode([-10,10],self.individuals[j][0])
            self.elitist['chromosome'][1] = self.decode([-10,10],self.individuals[j][1])
    ...
    def evolve(self):
        ...
        while True:
            ...
            #TODO 代码中的每次进化完成后用所有的后代去替换原有的染色体是不是过于简单粗暴？改成通过适应度函数的值去保留最优的50个染色体会不会更加准确一点？
