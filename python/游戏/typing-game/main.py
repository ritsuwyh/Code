...
speed = []  # speed列表存储每个字母的当前速度, 即speed[i]存储word[i]的速度. speed中每个元素是一个元组(speed_x, speed_y), 代表横向和纵向的速度
...
def hit_word(index):
    ...
    '''
    TODO: 1. 修改命中字母后的字母颜色, 可阅读graphic.py文件, 了解其中的get_hit_color()函数
          2. 维护连击次数combo. 
...
def action():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            for index in range(0, MAX_WORD_NUM):
                ...
                TODO: 键盘按下后, 判断按键是否与屏幕上未命中的字母相同, 即正在往下走的字母. 若出现多个相同的字母, 则返回屏幕中最靠下的字母的下标,
                        将所需要返回的字母的下标储存在 target_index 中.
                参数: key: 当前按下的字母; 
                ...
                '''
                # 在遍历的过程中找到最下面的目标的index
                if key == word[index] and state[index] == True:
                    if yy[index] >= max_depth:
                        max_depth = yy[index]
                        target_index = index
    ...
    for i in range(0, MAX_WORD_NUM):
        ...
        '''
        if state[i]==True:
            speed[i] = (speed[i][0] + acc, speed[i][1] + acc)
        else:
            speed[i] = (speed[i][0] - acc, speed[i][1] - acc)
...
# menu函数是程序主循环, 请勿修改!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# 欢迎阅读, 思考模块化的做法和特点. 整个打字游戏就是在这里跑起来的
def menu():
