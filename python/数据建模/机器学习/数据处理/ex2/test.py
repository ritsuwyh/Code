...
df = pd.read_csv('python\数据建模\机器学习\数据处理\song_data.csv')
...
#TODO https://zhuanlan.zhihu.com/p/131553804 series的使用 

#首先计算数据集df中除目标变量外的每个特征的唯一值数量，并将结果按照从小到大的顺序排序。
nu = df[features].nunique().sort_values()
#! series 可以通过自定义索引访问 也可以通过正常的索引访问  series.index[i] 就是获得索引i对应的自定义标签 series.value[i] 就是获取索引i对应的值 个人觉得没必要 直接访问就可以了
# print(nu)
# print(nu[1])
# print(nu['key'])
